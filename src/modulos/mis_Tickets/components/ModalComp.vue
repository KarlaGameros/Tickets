      ...
      <q-card-section>
        <q-form @submit="onSubmit">
          <div class="row q-col-gutter-sm">
            <div class="col-12" v-if="is_Editar || is_Visualizar">
              <q-badge
                :color="
                  ...
                  solicitud.estatus_Ticket == 'Pendiente de Cancelación'
                    ? 'orange'
            ...
            <div
              v-if="area_Informatica && !is_Visualizar"
              class="col-lg-4 col-md-4 col-sm-4 col-xs-12"
            >
              <q-select
                v-model="area_Inventario"
                use-input
                color="purple-ieen"
                input-debounce="0"
                :options="list_Area_Filtro"
                @filter="filterArea"
                label="Área inventario"
                hint="Seleccione área del inventario"
              >
                <template v-slot:prepend>
                  <q-icon color="purple-ieen" name="apartment" />
                </template>
            ...
            >
              <q-select
                ...
                color="purple-ieen"
                input-debounce="0"
                :options="list_Inventario_Filtro"
...
const { list_Areas, list_Personal_By_Area, list_Inventario } =
  storeToRefs(helpersStore);
const { solicitud, modal, is_Editar, is_Visualizar, inventario } =
  storeToRefs(misSolicitudesStore);
const puesto = ref(null);
const area_Id = ref(null);
const area_Inventario = ref(null);
...
const list_Inventario_Filtro = ref(list_Inventario.value);
const list_Area_Filtro = ref(list_Areas.value);
...
watch(area_Inventario, (val) => {
  if (val != null) {
    cargarInventarioByArea(val.value);
  }
});
...
watch(personal_Id, (val) => {
  if (val != null) {
    ...
    cargarInventarioByPersonal(val.id);
  }
});
...
const cargarInventarioByArea = async (id) => {
  loading();
  await helpersStore.load_Inventario_By_Area(id);
  list_Inventario.value.splice(0, 0, {
    value: 0,
    label: "Todos",
  });
  $q.loading.hide();
...
const cargarInventarioByPersonal = async (id) => {
  loading();
  await helpersStore.load_Inventario_By_Personal(id);
  $q.loading.hide();
};
...
const cargarInventarioFalla = async () => {
  ...
  if (inventario_Ticket.value.id != null) {
    if (inventario_Id.value.value == 0) {
      // let inventarioFiltrado = list_Inventario_By_Area.value.find(
      //   (x) => x.id == inventario_Ticket.value.inventario_Id
      // );
      let inventarioFiltrado = inventario_Ticket.value;
...
const filterInventario = (val, update) => {
  if (val === "") {
    update(() => {
      list_Inventario_Filtro.value = list_Inventario.value;
    });
    return;
  }
  update(() => {
    const needle = val.toLowerCase();
    list_Inventario_Filtro.value = list_Inventario.value.filter(
      (x) => x.label.toLowerCase().indexOf(needle) > -1
    );
  });
};

const filterArea = (val, update) => {
  if (val === "") {
    update(() => {
      list_Area_Filtro.value = list_Areas.value;
    });
    return;
  }
  update(() => {
    const needle = val.toLowerCase();
    list_Area_Filtro.value = list_Areas.value.filter(
      (x) => x.label.toLowerCase().indexOf(needle) > -1
    );
  });
};
...
const actualizarModal = (valor) => {
  ...
  misSolicitudesStore.updateModal(valor);
