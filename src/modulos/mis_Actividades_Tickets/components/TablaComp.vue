...
const { list_Seguimiento_Ticket, concluirSeguimiento } = storeToRefs(
...
const verSolicitud = async (row) => {
  ...
  ticketsSolicitudesStore.actualizarModalSeguimiento(true);
  ...
  if (row.observaciones == null || row.observaciones == "") {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (resp.success) {
          ticketsSolicitudesStore.load_Seguimiento_Ticket();
