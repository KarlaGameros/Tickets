    ...
    <q-card style="width: 900px; max-width: 90vw">
      ...
      <q-card-section>
        ...
        >
          ...
          >
            <div class="row q-col-gutter-sm">
              ...
              <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12">
                <q-input
                  readonly
                  label="Área a la que se le solicitó"
                  v-model="solicitud.area_Solicitud"
                />
              </div>
              <div class="col-12">
                <q-input
              ...
              >
                <q-btn
                  ...
                  label="Cambiar tipo de solicitud"
          ...
          <q-step
            :name="2"
            v-if="solicitud.estatus_Ticket != 'Pendiente'"
            title="Responsable"
          ...
          >
            <q-form @submit="onSubmit" class="row q-col-gutter-sm">
              ...
              <div
                class="col-12 text-center text-subtitle1 text-purple-ieen text-bold"
              >
              ...
              <q-separator class="col-12 q-mt-lg" />
              <div
                class="col-12 text-center text-subtitle1 text-purple-ieen text-bold"
              >
                Personal marcado con copia
              </div>
              <div
                v-if="!is_Visualizar"
                class="col-lg-6 col-md-6 col-sm-6 col-xs-12"
              >
                <q-select
                  stack-label
                  color="purple-ieen"
                  v-model="area_Copia_Id"
                  :options="list_Areas"
                  label="Área del personal"
              ...
              >
                <q-select
                  stack-label
                  color="purple-ieen"
                  v-model="personal_Copia_Id"
                  :options="list_Personal_By_Area_Copia"
                  label="Personal responsable"
                  hint="Seleccione personal"
                >
                  <template v-slot:prepend>
              ...
              <div v-if="!is_Visualizar" class="col-12 text-right q-pt-lg">
                <q-btn
                  size="sm"
                  @click="agregarConCopia"
                  label="Agregar"
                  icon-right="add"
                  color="grey"
                ></q-btn>
                      ...
                      >
                        <div v-if="col.name === 'id'">
                          <q-btn
                            ...
                            color="red"
                            icon="delete"
                            @click="eliminarConCopia(props.row)"
                          >
...
const { list_Con_Copia, copia } = storeToRefs(conCopiaStore);
const { solicitud } = storeToRefs(misSolicitudesStore);
const { inventario_Ticket } = storeToRefs(inventarioStore);
const { list_Personal_By_Area, list_Areas, list_Personal_By_Area_Copia } =
...
const area_Copia_Id = ref(null);
const categoria_Id = ref(null);
const subcategoria_Id = ref(null);
const personal_Copia_Id = ref(null);
...
const area_Informatica = ref(encryptStorage.decrypt("area"));
...
watch(area_Copia_Id, (val) => {
  if (val != null) {
    cargarPersonalCopia(val);
  }
});
...
const cargarPersonalCopia = async (val) => {
  await helpersStore.load_Personal_By_Area_Copia(val.value);
};
...
const agregarConCopia = async () => {
  let resp = null;
  copia.value.solicitud_Id = solicitud.value.id;

  if (personal_Copia_Id.value == null) {
    alertNotify("center", "warning", "No seleccionó personal");
  } else {
    let filtro = list_Con_Copia.value.find(
      (x) => x.empleado_Id == personal_Copia_Id.value.value
    );
    if (filtro == undefined) {
      copia.value.empleado_Id = personal_Copia_Id.value.value;
      resp = await conCopiaStore.create_Con_Copia(copia.value);
      if (resp.success) {
        personal_Copia_Id.value = null;
        conCopiaStore.initCopia();
        await conCopiaStore.load_Con_Copia_By_Solicitud(solicitud.value.id);
        alertNotify("top-right", "positive", resp.data);
      } else {
        alertNotify("top-right", "negative", resp.data);
      }
      ...
      $q.notify({
        position: "center",
        type: "warning",
        message: "El personal seleccionado ya se agregó anteriormente",
      });
    }
  }
};

const eliminarConCopia = async (row) => {
  $q.dialog({
    title: "Eliminar con copia",
    message: `¿Está seguro de eliminar con copia del personal ${row.empleado}?`,
    icon: "Warning",
    persistent: true,
    transitionShow: "scale",
    transitionHide: "scale",
    ...
    loading();
    let resp = await conCopiaStore.eliminar_Con_Copia(row.id);
    if (resp.success) {
      await conCopiaStore.load_Con_Copia_By_Solicitud(row.solicitud_Id);
      alertNotify("top-right", "positive", resp.data);
    } else {
      alertNotify("top-right", "negative", resp.data);
    }
...
const actualizarModal = (valor) => {
  limpiar();
  ticketsSolicitudesStore.actualizarModalSolicitud(valor);
...
const onSubmit = async () => {
  ...
  responsable.value.tipo_Solicitud_Id = list_Tipos_Solicitudes.value.find(
    (x) => x.label == "Solicitudes"
  ).value;
  responsable.value.subcategoria_Id = subcategoria_Id.value.value;
  let resp = await ticketsSolicitudesStore.asignar_Responsable_Ticket(
    solicitud.value.id,
    responsable.value
  );
  if (resp.success) {
