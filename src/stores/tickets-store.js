...
export const useTicketsStore = defineStore("useTicketsStore", {
  state: () => ({
    modal: false,
    modal_Archivo: false,
  ...
  actions: {
    ...
    async load_Solicitudes_Tickets() {
      try {
        ...
        if (resp.status == 200) {
          ...
          if (success == true) {
            if (data) {
              this.list_Solicitudes_Tickets = data.map((item) => {
                return {
                  id: item.id,
                  folio: item.folio,
              ...
              this.list_Solicitudes_Tickets.sort((a, b) => b.id - a.id);
            }
          }
        } else {
          return {
    ...
    async load_Seguimiento_Ticket() {
      try {
        ...
        const resp = await api.get("/T_Detalles_Solicitudes");
        if (resp.status == 200) {
          const { success, data } = resp.data;
          if (success == true) {
            if (data) {
              this.list_Seguimiento_Ticket = data.map((item) => {
                return {
                  id: item.id,
                  tipo: item.tipo,
              ...
              this.list_Seguimiento_Ticket.sort((a, b) => b.id - a.id);
            }
          }
        } else {
          return {
      ...
      try {
        ...
        if (resp.status == 200) {
          ...
          if (success === true) {
            ...
            this.seguimiento.fecha_Fin_Seguimiento = data.fecha_Fin_Seguimiento;
          } else {
            return { success, data };
          }
        } else {
